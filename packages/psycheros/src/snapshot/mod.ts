 ...
 */
export const IDENTITY_CATEGORIES: SnapshotCategory[] = [
  "self",
  "user",
  "relationship",
  "custom",
...
function parseSnapshotFilename(
  ...
  return {
    ...
    source: "psycheros" as SnapshotSource,
  };
}

/**
 * Read the header of a snapshot file to extract reason and source.
 * Only reads the first 512 bytes to avoid loading full file content.
 */
...
export async function getSnapshotContent(
  ...
  const idWithoutExtension = snapshotId.replace(/\.md$/, "");

  const match = idWithoutExtension.match(/^(.+)\/(.+)$/);
  if (!match) {
...
export async function restoreSnapshot(
  ...
  if (!contentResult.success || !contentResult.content) {
    return {
      success: false,
      message: contentResult.error || "Failed to read snapshot",
  ...
  try {
    if (mcpClient) {
      await mcpClient.writeIdentityFile(
        category,
        filename,
        content,
        projectRoot,
    ...
    return {
      success: true,
      message: `Restored ${category}/${filename} from snapshot`,
    };
  } catch (error) {
