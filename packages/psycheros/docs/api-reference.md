...
| `POST` | `/api/admin/entity-data/export`                   | Export all entity data as zip. Returns zip on success, or JSON `{ partial: true, error, message }` if entity-core is unavailable                                                                                                          |
| `POST` | `/api/admin/entity-data/export?partial=1`         | Export Psycheros-only data (skips entity-core collection)                                                                                                                                                                                 |
| `POST` | `/api/admin/entity-data/import`                   | Import entity data from uploaded zip (body = raw zip bytes). Streaming NDJSON response with phases: `validate` → `conversations` → `lorebooks` → `vault` → `images` → `anchors` → `entity-core` → `restart` → `sync` → `cleanup` → `done` |
